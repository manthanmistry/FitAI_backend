...
def log_weight(payload: WeightLogIn, db: Session = Depends(get_db)):
...
def log_workout(payload: WorkoutLogIn, db: Session = Depends(get_db)):
...
def log_water(payload: WaterLogIn, db: Session = Depends(get_db)):
    _get_user_or_404(db, payload.user_id)
    log = models.WaterLog(user_id=payload.user_id, amount_l=payload.amount_l)
    db.add(log)
    db.commit()
    return {"status": "logged"}
...
def get_progress(user_id: int = Query(...), db: Session = Depends(get_db)):
    ...
    # workout streak: consecutive days (including today) with at least one workout log
    streak = 0
    if workout_logs:
